...
```bash
git clone https://iramiller.com/src/js/stream/.git stream
```

or with npm

```bash
npm install --save @guld/stream
```
...
`@guld/stream` is a fork of `readable-stream`, which is maintained by the Streams Working Group, which
